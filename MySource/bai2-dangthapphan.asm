...
; Cac cau lenh cua chuong trinh
    ...
    CALL INDEC        ; Goi den ham con Nhap
    MOV A, AX        ; Dua so vua nhap vao A
     
    MOV AH, 9
    LEA DX, TBAO2
    INT 21H          ; In ra thong bao nhap so 2
    CALL INDEC
    MOV B, AX        ; ;Dua so vua nhap vao B
    
    MOV AH, 9
    LEA DX, TBAO3
    INT 21H          ; In ra thong bao nhap so 3
    CALL INDEC
    MOV C, AX        ; Dua so vua nhap vao C
    
    MOV AH, 9
    LEA DX, TBAO5
    INT 21H          ; In ra thong bao chu vi
    CALL ChuVi       ; Goi den ham con tinh chu vi
    CALL OUTDEC     ; Goi den ham con Hien thi
Lap:
